<template>
  <div class='text-gray-800'>
    ...
    <div class='ml-4 mr-4 mt-7 '>
      <div>
        <Card v-for='video in filteredVideos' :key='video._id' :video='video' />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  async created() {
    try {
      ...
      console.log('get all videos error', err)
		...
		this.filteredVideos = this.videos.filter(()=>{return true})
  },
  ...
  methods: {
		filterVideos() {
			...
			this.filteredVideos = this.videos.filter((e, i) => {
				return this.videos[i].title.toLowerCase().includes(this.searchQuery.toLowerCase())
			})}
		},
		search() {
			this.filterVideos();
